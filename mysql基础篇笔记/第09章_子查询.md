...
> 相关子查询的需求：查询工资大于**本部门**平均工资的员工信息
>
> 不相关子查询的需求：查询工资大于**本公司**平均工资的员工信息
...
WHERE manager_id IN (SELECT manager_id
  ...
  AND department_id IN (SELECT department_id
                        FROM employees
                        WHERE employee_id IN (141, 174))
  AND employees.employee_id NOT IN (141, 147);
...
HAVING AVG(salary) = (SELECT MIN(avg_sal)
                      FROM (SELECT AVG(salary) avg_sal
                            FROM employees
                            GROUP BY department_id) AS alias);
```

```mysql
#方式2，小于等于所有的，也就是等于最小的那个
SELECT department_id
FROM employees
GROUP BY department_id
...
WHERE employee_id NOT IN (
			SELECT manager_id
			FROM employees # 查出来有NULL值，结果为空
    		# where manager_id IS NOT NULL 加上这句就有结果
			);
```
...
- 因为在NOT IN里进行不相等比较，任意 !=null 的运算结果都是false，所以输出不了任何值
- 举例解答一：`8 NOT IN (20,NULL)`， 本质上进行的运算是 `8!=20 AND 8!=NULL`, 即 `1 AND NULL`, 所以返回 NULL
- 举例解答二：`1 NOT IN (1, NULL)`， 本质上进行的运算是 `1!=1 AND 1!=NULL`, 即 `0 AND NULL`, 所以返回 0( 逻辑与运算符：当给定的一个值或者多个值为0时则返回0；)
- 举例解答三：由此可以推广，对于a NOT IN (...,...,...NULL), 不会查询到任何结果，所以使用NOT IN要避免查询列表出现NULL
...
如果子查询的执行**依赖于外部查询**，通常情况下都是因为子查询中的表用到了外部的表，并进行了条件关联，因此每执行一次外部查询，子查询都要重新计算一次，这样的子查询就称之为`关联子查询`。
...
**方式二：在 FROM 中使用子查询**：将所有部门的平均工资作为一张临时表
