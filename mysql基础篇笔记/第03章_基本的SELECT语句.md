...
- **推荐采用统一的书写规范：**
  ...
  - 单引号内严格意义上是需要区分大小写的（`COLLATE utf8_general_ci`来设置大小写不敏感的排序规则，`utf8_general_ci`表示以General CI（不区分大小写）为排序规则。MySQL还提供其他的COLLATE参数，用于满足不同语言、字符编码的需求，例如`utf8_bin`用于区分大小写。）
  
    - ```
      SELECT *
      FROM employees
      WHERE last_name = 'king';  //可以查询出结果
      ```
  
    * ![image-20230825113458134](C:\Users\17607\AppData\Roaming\Typora\typora-user-images\image-20230825113458134.png)
...
https://www.maitanbang.com/blog/detal/?id=5995
...
-- DAUL为伪表，dual表就是oracle与数据字典自动创建的一张表，这张表是一个单行单列的表，这个表只有1列：DUMMY，数据类型为VERCHAR2(1)，dual表中只有一个数据'X', Oracle有内部逻辑保证dual表中永远只有一条数据。dual表主要是用来选择系统变量或是求一个表达式的值

/*
原文链接：https://blog.csdn.net/weixin_39832643/article/details/113281547
*/
```
...
我们需要保证表中的字段、表名等没有和保留字、数据库系统或常用方法冲突。如果真的相同，请在SQL语句中使用一对``（着重号，键盘左上角那个）引起来。
...
![image-20230825093808030](C:\Users\17607\AppData\Roaming\Typora\typora-user-images\image-20230825093808030.png)
