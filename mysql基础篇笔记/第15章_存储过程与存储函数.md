...
**举例4：**创建存储过程show_min_salary()，查看“emps”表的最低薪资值。并将最低薪资**通过OUT参数“ms”输出**

```mysql
DELIMITER //

CREATE PROCEDURE show_min_salary(OUT ms DOUBLE)  #需要和表中的类型保持一致
	BEGIN
...
CALL show_min_salary(@ms);  -- 在调用存储过程的时候，定义变量
SELECT @ms;  -- 查看变量值
```



**举例5：**创建存储过程show_someone_salary()，查看“emps”表的某个员工的薪资，并用**IN参数empname输入员工姓名**。
...
**举例6（==输出有两个参数==）：**创建存储过程show_someone_salary2()，查看“emps”表的某个员工的<u>薪资和住址</u>，并用IN参数empname输入员工姓名，用OUT参数empsalary输出员工薪资，OUT参数 location输出员工住址。

```mysql
DELIMITER //

CREATE PROCEDURE show_someone_salary2(IN empname VARCHAR(20),OUT empsalary DOUBLE, OUT location VARCHAR(20))
	BEGIN
		SELECT t1.salary,t1.location INTO empsalary,location   -- 参数有多个，不需要用括号括起
		FROM emps t1    -- 给表取别名 
		WHERE t1.empname = empname;  -- 参数和表中的字段重名，需要指明具体的表
	END //
...
![image-20230909210722954](C:\Users\17607\AppData\Roaming\Typora\typora-user-images\image-20230909210722954.png)
...
[LIKE 'pattern']：使用模糊查询匹配存储过程或函数的名称，可以省略。当省略不写时，会列出MySQL数据库中存在的所有存储过程或函数的信息。
...
![image-20230909211457122](C:\Users\17607\AppData\Roaming\Typora\typora-user-images\image-20230909211457122.png)
...
WHERE ROUTINE_NAME='存储过程或函数的名' [AND ROUTINE_TYPE = {'PROCEDURE|FUNCTION'}]; -- 注：此处一定要大写。
```
...
修改存储过程或函数，不影响存储过程或函数功能，只是修改相关特性【characteristic...】。使用ALTER语句实现。
