...
| ROUND(x,y)          | 返回一个对x的值进行四舍五入后最接近X的值，并保留到小数点后面Y位，负数则表示小数点前 |
...
| LPAD(str, len, pad)               | 用字符串pad(中文也可以）对str最左边进行填充，直到str的长度为len个字符 |
...
SELECT  DATE_ADD(NOW(), INTERVAL 1 DAY) AS col1,
		...
		DATE_ADD('2021-10-21 23:32:12',INTERVAL '1_1' MINUTE_SECOND) AS col4, #在分钟和秒钟都加上
		DATE_ADD(NOW(), INTERVAL -1 YEAR) AS col5, #可以是负数，表示减
...
| GET_FORMAT(date_type,format_type) | 返回日期字符串的显示格式，返回一个格式，可用于上面两个中的fmt的位置,<br>`SELECT DATE_FORMAT(CURDATE(),GET_FORMAT(DATE,'USA'))FROM DUAL;` |
...
一般都是在前端或者后端进行加密，不在数据库中加密。

加密与解密函数主要用于对数据库中的数据进行加密和解密处理，以防止数据被他人窃取。这些函数在保证数据库安全时非常有用。

| 函数                                       | 用法                                                         |
| ------------------------------------------ | ------------------------------------------------------------ |
| PASSWORD(str)，（8.0中弃用）               | 返回字符串str的加密版本，41位长的字符串。加密结果`不可逆`，常用于用户的密码加密 |
| MD5(str)                                   | 返回字符串str的md5加密后的值，也是一种加密方式。若参数为NULL，则会返回NULL。加密结果`不可逆` |
| SHA(str)                                   | 从原明文密码str计算并返回加密后的密码字符串，当参数为NULL时，返回NULL。`SHA加密算法比MD5更加安全`。 |
| ENCODE(value,password_seed)，（8.0中弃用） | 返回使用password_seed作为加密密码加密value                   |
| DECODE(value,password_seed)，（8.0中弃用） | 返回使用password_seed作为加密密码解密value                   |
