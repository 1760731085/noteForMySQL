...
```mysql
#方式2：
alter table 表名称 modify 字段名 字段类型 unique;  #字段类型也要写	
```
...
```mysql
ALTER TABLE USER 
ADD CONSTRAINT uk_name_pwd UNIQUE(NAME,PASSWORD); #可以取别名
```
...
- 如果创建唯一约束时未指定名称，如果是单列，就默认和列名相同；如果是组合列，那么*默认和()中排在第一个的列名相同*。也可以自定义唯一性约束名。
...
（1）一个表中只能有一个字段使用 AUTO_INCREMENT 约束，且该字段必须有唯一索引，以避免序号重复（即为主键或主键的一部分）。

（2）当需要产生唯一标识符或顺序值时，可设置自增长。默认情况下，AUTO_INCREMENT 的初始值是 1，每新增一条记录，字段值自动加 1。

（3）自增长列约束的列必须是键列（主键列，唯一键列）

（4）自增约束的列的数据类型必须是整数类型（TINYINT、SMALLINT、INT、BIGINT 等）

（5）如果向自增列添加了 0 和 null（没有填写该字段的值），实际上会自动在当前最大值的基础上自增；如果自增列手动指定了具体值，直接赋值为具体值。

```mysql
INSERT INTO test7(id,last name)
VALUES(0,'Tom');  -- 在表中该记录的id值并不为0，为当前最大值+1
```



> 开发中，一旦主键作用的字段上声明有AUTOINCREMENT，则我们在添加数据时，就不要给主键对应的字段去赋值了。
...
（7）从表的外键列与主表被参照的*<u>列名字可以不相同，但是数据类型必须一样，逻辑意义一致</u>*。如果类型不一样，创建子表时，就会出现错误“ERROR 1005 (HY000): Can't create table'database.tablename'(errno: 150)”。
...
* `No action方式`（拒绝策略）：如果子表中有匹配的记录，则不允许对父表对应候选键进行update/delete操作  
...
不建外键约束，你的操作（创建表、删除表、添加、修改、删除）不受限制，要保证数据的`引用完整性`，只能依`靠程序员的自觉`，或者是`在Java程序中进行限定（将可以添加的值都放在集合中，如果当前添加的值在集合中存在，则添加；否则不予执行`。例如：在员工表中，可以添加一个员工的信息，它的部门指定为一个完全不存在的部门。
...
总的来说，建立索引的约束可以表级约束且用ADD修改，而不建立索引的用MODIFY修改且没办法进行表级约束