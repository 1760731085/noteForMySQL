...
如果MySQL中已经存在相关的数据库，则忽略创建语句，不管后面的字符集是否修改，不再创建数据库。
...
```mysql
SELECT DATABASE() FROM DUAL;  #使用的一个 mysql 中的全局函数
```
...
- **可选指定：**
  - 约束条件
  - 默认值
  - 如果创建表时没有指明使用的字符集，则认使用表所听在的数据库的字符集
...
```mysql
CREATE TABLE emp1 AS SELECT * FROM employees; -- 创建的emp1，将employees表的数据完全拷贝过去。

CREATE TABLE emp2 AS SELECT * FROM employees WHERE 1=2; --过滤条件写为假，创建的emp2是空表，不复制数据。
```
...
```mysql
DESCRIBE dept80;  #新表中共有四个字段，同子查询显示的字段数相同。且字段名和查询语句中起的别名一致。
```
...
**注意：**此方式创建表不会把除了非空约束外的约束一起复制到新表中。
...
```mysql
ALTER TABLE 表名 ADD 【COLUMN】 字段名 字段类型 【FIRST|AFTER 字段名】; #【FIRST|AFTER 字段名】指明字段添加到哪个具体的位置
```
...
- 修改字段数据类型（一般不改）、长度(如果原有的数据长度超过要修改的长度，就会修改失败）、默认值、位置的语法格式如下：
...
> 阿里开发规范：
>
> 【参考】TRUNCATE TABLE 比 DELETE 速度快，且使用的系统和事务日志资源少，但 TRUNCATE 无事务且不触发 TRIGGER，（不会在数据被删除的时候进行备份）有可能造成事故，故不建议在开发代码中使用此语句。 
>
> 说明：TRUNCATE TABLE **在功能上**与不带 WHERE 子句的 DELETE 语句相同。
